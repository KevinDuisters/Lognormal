...
for(l in 1:L){
  ...
  D[l,] <- c(qnorm(0.10,mu,sigma),qnorm(0.90,mu,sigma))
...
MDR <- function(omega){ifelse(omega>=1,1/omega*exp((1/2)*(omega^2-1)),NA)}
...
rel90<-(D[,2]-A[,2])/A[,2]
rel90h <- (exp(omegaseq^2/2) + qnorm(0.9)*sqrt(exp(2*omegaseq^2)-exp(omegaseq^2)))/exp(qnorm(0.9)*omegaseq)-1
cbind(rel90,rel90h)
...
normf <- function(vec){
  return((max(vec,na.rm=T)-vec)/(max(vec,na.rm=T)-min(vec,na.rm=T)))
}
...
sub10 <- rep(T,L)
plot(x=rel10[sub10],y=normf(MDR(omegaseq[sub10])),ylab="Rescaled stat",xlab="rel diff",type="l",ylim=c(0,1),main="10",xlim=c(-2,0))
...
sub90 <- rep(T,L)
plot(x=rel90[sub90],y=normf(MDR(omegaseq[sub90])),ylab="Rescaled stat",xlab="rel diff",type="l",ylim=c(0,1),main="90",xlim=c(0,2))
